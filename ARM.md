...
> 记忆方法：将CPSR或者SPSR简记为S，M记为mov，R记为寄存器（RN），且读取指令的顺序为从右到左，则MSR为从寄存器中将数据加载到状态寄存器中；MRS则是将状态寄存器的值加载到寄存器中。
...
LDC和STC记忆更方便，已知LD为加载指令，加个C就是加载C中的数据。ST为存储指令，加个C就是将数据加载到C中。

#### 异常处理指令
&nbsp;&nbsp;异常处理指令有：<br>
SWI，BKPT, RFE<br>
SWI指令为软中断指令，通过设置软中断来让软件自陷，来切换优先级来调用操作系统的一部分高级服务，直观的就是通过调用SWI指令来让软件进行软中断。<br>
> 指令格式为：SWI{< cond >} < SWI number ><br>
其中的< cond >为条件域，< SWI number >为软中断号。<br><br>
记忆方法： SW-》swap， I-》Interrupt，连接起来就是：转换到中断位置执行

BKPT指令为断点指令，此指令被调用时将触发一个预取指令异常，将使处理器进入调试状态。
> 指令格式为：BKPT <#imm16><br>
在触发中断后，#imm16这个立即数的值将会被忽略。<br><br>
记忆方法： BKPT-》BreaKPoinT

REF指令可从异常中返回，指令将Rn所指的内容加载到pc和CPSR寄存器，可选择是否对Rn进行自增回写操作。
> 语法:	RFE< DA|DB|IA|IB > Rn{!}<br>
DA.DB、IA、IB和之前的一样，Rn如果加上“！”则代表在执行完此指令后Rn自增一位。

#### 指令执行顺序
ARM指令通过添加一些后缀标志信息来做到改变指令顺序而且节省指令执行数目，例如在指令后添加指令条件码可以省去不少的判断跳转指令，所以除了跳转指令外，分支指令也可以改变指令的执行顺序和效果。<br><br>
此外，还有前序寻址和后序寻址也会改变指令的执行顺序。
> 以STR指令为例:<br>
指令STR R0， [R1, #12]达到的效果为先将R1的地址先增加12，之后再在新的地址位置所指的内存中取出值放在R0中。即先将地址增加后再取值赋值。<br><br>
指令STR R0， [R1], #12达到的效果又有所不同，此指令的动作为将R1所存的内容作为地址，取出该地址所指的内存所存储的值后将值放入R0，之后将R1增加12.即先赋值后再修改地址的位置<br><br>
要注意的是，“STR R0， [R1], #12”这条指令在执行结束后，R1的值会发生变化，而“STR R0， [R1, #12]”指令在执行后，R1的值并不会发送变化，原来是多少就是多少，如果想让R1的值在执行后发生变化，则需要在括号外添加“！”，变为：“STR R0， [R1, #12]！”这样指令执行后R1的值将会变化。<br><br>
直观解释：![change](Change.jpg)
